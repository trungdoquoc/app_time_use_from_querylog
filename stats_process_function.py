...
def query_client_log_relative_time(url: str, port: int, group_id: str, client_id: str) -> list:
...
def query_workspace_logs_relative_time(url: str, port: int, group_list: list) -> list:
	"""
	Note: 
...
def get_time_use_one_client(url: str, port: int, group_id: str, client_id: str):
	client_logs = query_client_log_relative_time(
		url=url, port=port, group_id=group_id, client_id=client_id)
...
def get_time_use_workspace(url: str, port: int, group_list: list):
	"""
	input: 
...
if __name__ == "__main__":
	...
	t_group = "group-9b1956eb-ad3b-4427-a489-34a3fd8ffcb5"
	t_client = "bdnerkwszesl"
	t_workspace = ["group-9b1956eb-ad3b-4427-a489-34a3fd8ffcb5", "group-visafe"]
	port = 9200

	# 1 CLIENT:
	new_client = get_time_use_one_client(elk_address, port, t_group, t_client)
	print(new_client.get_record().get_category_stats_time())
	print(new_client.get_record().get_app_stats_time())

	# 1 GROUP:
	new_group = get_time_use_one_group(url=elk_address, port = port, group_id=t_group)
	print(new_group.get_group_record().get_category_stats_time())
	print(new_group.get_group_record().get_app_stats_time())

	# MULTIPLE GROUPS:
	new_workspace = get_time_use_workspace(elk_address, port, t_workspace)
